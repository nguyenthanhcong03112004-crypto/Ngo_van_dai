...
async function loadDashboardData() {
    try {
        ...
        const response = await fetch('http://localhost:8888/api/admin/analytics', {
...
async function loadRecentReviews() {
    ...
    try {
        ...
        const response = await fetch('http://localhost:8888/api/admin/reviews/recent', {
            headers: { 'Authorization': `Bearer ${user.token}` }
        });
        const result = await response.json();
            ...
            container.innerHTML = result.data.map(r => {
                ...
                return `
                    <div class="pt-6 first:pt-0 flex gap-4">
                        <img src="${r.avatar_url ? 'http://localhost:8888' + r.avatar_url : 'https://picsum.photos/seed/user' + r.user_id + '/100/100'}" onerror="this.src='https://picsum.photos/seed/user${r.user_id}/100/100'" class="w-10 h-10 rounded-full border-2 border-slate-50 shadow-sm object-cover shrink-0">
                        <div class="flex-1">
...
async function deleteReview(id) {
    ...
    try {
        const user = AuthManager.getUser();
        const response = await fetch(`http://localhost:8888/api/admin/reviews/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${user.token}` }
        });
        const result = await response.json();
